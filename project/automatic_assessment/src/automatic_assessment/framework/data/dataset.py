...
class Dataset(TorchDataset):
    ...
    def create_augmented_and_filtered_dataset(self, selected_targets, augmentation_ratio: int, apply_lasso: bool, subfolder: str, apply_lars: bool = False, max_n_timeseries: int = 20):
        """
        ...
        # 2. Multi-Target LARS Feature Selection (Optional)
        if apply_lars:
            print(f"Applying Multi-Target LARS selection (Top {max_n_timeseries} sources)...")
            
            # Columns corresponding to time-series features
            timeseries_cols = Preprocessor.TIMESERIES_IMPUTE_WITH_ZERO_COLS + Preprocessor.TIMESERIES_IMPUTE_WITH_CLOSEST_COLS
            extracted_ts_feature_cols = [c for c in self.path_related_df.columns if get_source_name(c, timeseries_cols) is not None]
            print(f"  Identified {len(extracted_ts_feature_cols)}/{len(self.path_related_df.columns)} time-series derived features for LARS selection.")

            # Flatten all paths that belong to one user into separate columns
            # Pivot table to get one row per user: index=user, columns=path, values=feature_cols
            path_df_subset = self.path_related_df[['user', 'path'] + extracted_ts_feature_cols]
            pivoted = path_df_subset.pivot(index='user', columns='path', values=extracted_ts_feature_cols)
            
            # Flatten the MultiIndex columns to single string: feature_name + "_path_" + path_id
            pivoted.columns = [f"{col[0]}_path_{col[1]}" for col in pivoted.columns]
            pivoted = pivoted.reset_index()
            
            # Merge ensures we align X and y by user
            aligned_df = pd.merge(pivoted, self.target_df, on='user', how='inner')
            
            # Extract X (features) and y (targets)
            X_cols = [c for c in pivoted.columns if c != 'user']
            X = aligned_df[X_cols]
            
            y_cols = [c for c in self.target_df.columns if c != 'user']
            y_df = aligned_df[y_cols]
            
            # Scale features and targets for LARS selection (important: do not modify self.* dataframes)
            scaler_X = StandardScaler()
            X_scaled = pd.DataFrame(scaler_X.fit_transform(X), columns=X.columns)
            
            scaler_y = StandardScaler()
            y_scaled = pd.DataFrame(scaler_y.fit_transform(y_df), columns=y_df.columns)
            
            # Run Multi-Target LARS to select best time-series sources
            selected_sources = select_multitarget_top_sources_lars(X_scaled, y_scaled, timeseries_cols, top_n_sources=max_n_timeseries)
