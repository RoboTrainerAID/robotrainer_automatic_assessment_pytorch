...
class MLPSharedEncoderFLAT(BaseModel):
    model_name = "MLP_SharedEncoderFLAT"
    ...
    def __init__(self, input_dims, output_dim, hyperparams):
        ...
        # Regressor input depends on pooling strategy:
        # - "mean" / "attention": (embed_dim + f_user)
        # - "flatten": (embed_dim * n_paths + f_user)
        if self.pooling == "flatten":
            regressor_input_dim = self.embed_dim * self.n_paths + self.f_user
        else:
            regressor_input_dim = self.embed_dim + self.f_user

        self.regressor = nn.Sequential(
            nn.Linear(regressor_input_dim, self.reg_hidden_dim),
            nn.ReLU(),
            nn.Dropout(self.dropout),
    ...
    def forward(self, x: List[torch.Tensor]) -> torch.Tensor:
        ...
        if self.pooling == "mean":
            ...
            # Flatten all path embeddings into a single vector: (B, P * embed_dim)
            pooled = embeddings.view(B, P * self.embed_dim)

        else:
            raise ValueError(f"Invalid pooling strategy: '{self.pooling}'. Choose 'mean', 'attention' or 'flatten'.")
