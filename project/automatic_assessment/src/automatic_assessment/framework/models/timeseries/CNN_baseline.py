...
class CNNBaseline(BaseModel):
    ...
    def __init__(self, input_dims, output_dim, hyperparams):
        ...
        self.cnn = nn.Sequential(
            nn.Conv1d(self.ts_features, channels, kernel_size=5, padding=2),
            nn.ReLU(),
    ...
    def encode_timeseries(self, x_ts, lengths):
        B, P, TS, T, F = x_ts.shape

        x = x_ts.reshape(B * P * TS, T, F).permute(0, 2, 1)
        ...
        pooled = pooled.reshape(B, P * TS * pooled.shape[-1])
        return pooled

    # -----------------------------------------------------

    def forward(self, x: List[torch.Tensor]) -> torch.Tensor:
        x_ts, x_path, x_user, lengths = x

        ts_feat = self.encode_timeseries(x_ts, lengths)
