...
./docker/build.sh \
    $NO_CACHE_FLAG \
    $DEBUG \
    -b "USERIMAGE=$DOCKER_NAMESPACE/$DOCKER_NAME:$DOCKER_TAG-base $DOCKER_USER_ARGS TORCH_CUDA_ARCH=$TORCH_CUDA_ARCH MAX_BUILD_JOBS=$MAX_BUILD_JOBS" \
    -f ./docker/Dockerfile \
    -t $DOCKER_NAMESPACE/$DOCKER_NAME:$DOCKER_TAG-dev